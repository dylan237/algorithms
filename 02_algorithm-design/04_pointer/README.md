...
**目標： 實作一個函數，第一個參數接收一個 sorted array of numbers；第二個參數接收一個數字。並回傳陣列內兩個數相加除以 2 等於第二參數的所有 pairs**
...
Pointer 透過指定兩個指標，left 及 right，從陣列中最大與最小開始進行配對，left 一開始為 0，即指向陣列中最小的數字，right 指向陣列最後一個元素，即最大數字。
從這兩個元素開始進行計算，若相加除以 2 比 average 還大，則代表數字過大，須將 right 指標往前遞減一個索引，使得新的數字更小；反之則是 left 指標遞增一個索引，使數字更大。
...
============ STEP 1 ==============
 left                       right
  |                           |
[-11, 0, 1, 2, 3, 9, 14, 17, 21]    -->   (-11 + 21) / 2 = 5 != 1.5 --> ❌ TOO BIG --> right--

============ STEP 2 ==============
 left                   right
  |                       |
[-11, 0, 1, 2, 3, 9, 14, 17, 21]    -->   (-11 + 17) / 2 = 3 != 1.5 --> ❌ TOO BIG --> right--

============ STEP 3 ==============
 left               right
  |                   |
[-11, 0, 1, 2, 3, 9, 14, 17, 21]    -->   (-11 + 14) / 2 = 1.5 == 1.5 --> ✅ Insert pair to result array && right-- && left++

============ STEP 4 ==============
     left       right
      |           |
[-11, 0, 1, 2, 3, 9, 14, 17, 21]    -->   (0 + 9) / 2 = 4.5 !== 1.5 --> ❌ TOO BIG --> right--

============ STEP 5 ==============
     left    right
      |        |
[-11, 0, 1, 2, 3, 9, 14, 17, 21]    -->   (0 + 3) / 2 = 1.5 == 1.5 --> ✅ Insert pair to result array && right-- && left++

============ STEP 6 ==============
      left  right
         |  |
[-11, 0, 1, 2, 3, 9, 14, 17, 21]    -->   (1 + 2) / 2 = 1.5 == 1.5 --> ✅ Insert pair to result array && right-- && left++
```
