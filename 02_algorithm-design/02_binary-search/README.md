...
Binary Search 使用的先決條件是，input 必須是 sorted 的資料，透過 sorted 的特性，從資料的中間元素開始查找，並判斷 target 在他的左半部或是右半部，近而排除一半的資料，縮小搜尋範圍。
...
我要要在這堆數字中找到數字 `98`，用[簡易搜尋(Linear Search)](https://github.com/dylan237/algorithms-data-structures-notes/blob/master/01_algorithm-design/01_linear-search/fundamental.md)的方式就是從頭到尾一個一個搜尋，在這個例子中找到目標時，已經花費 7 步。
二分搜尋善用`已排序`的特性，他從陣列中間點開始找，如果中間的元素小於查詢數字的話，可以得知，我想找的數字肯定在右半邊陣列中，接著再從右半邊陣列取中間元素，重複步驟不斷縮小範圍，直到找到目標，這麼一來每執行一個步驟，都可以排除一半的 input size，效率比簡易搜尋快得多。
