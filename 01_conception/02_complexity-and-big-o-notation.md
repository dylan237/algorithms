...
> 此章節部分內容取自[App Works School](https://medium.com/appworks-school/tagged/algorithms) 的程式麻瓜的程式知識課系列，如不妥請通知。
...
比較標準的方式去判斷演算法優劣，是透過`複雜度`來評估，複雜度又分為兩種，`時間複雜度 Time Complexity` 與 `空間複雜度 Space Complexity`。前者評估為程式執行的時間；後者評估程式所佔用的記憶體大小。接下來的內容主要會把重點放在`時間複雜度`的部分。
...
假設你今天心血來潮，想要重看經典電影名著《鐵達尼號》。你有兩個方法：
...
在 Big O 中，常數可以直接被去除，原因是它只考慮 n 為無窮大時的情況，相較於 n 的無窮大，常數對於演算法的的影響相對不大。
將 3 去除後時間複雜度為 `f(n) = n`，以 Big O 來描述則記作 `O(n)`
